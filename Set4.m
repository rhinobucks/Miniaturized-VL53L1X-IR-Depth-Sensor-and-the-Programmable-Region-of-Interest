...
test_name='Determining Wavelength of IR Optical Sensor';
...
ambient_light= 0; %Ambient light in lx
led_current=0; %LED current in Amps

target_notes='Target Reflectivity: White (88%)'; %Reflectivity or other notes here 

%%%%%%%%% Setting Entry %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
itn=5000; %Number of iterations of grabbing data from sensor 
...
file_data = fopen('SensorData.txt','a');
fprintf(file_data, 'Average Time Elapsed: %s sec', num2str((total_time)/(itn)));
fclose(file_data);
dlmwrite('SensorData.txt',space1,'delimiter',' ','newline', 'pc','-append')
...
file_data = fopen('SensorData.txt','a');
fprintf(file_data, 'Number of Samples Taken Total: %d samples',itn);
fclose(file_data);
dlmwrite('SensorData.txt',space1,'delimiter',' ','newline', 'pc','-append')
...
file_data = fopen('SensorData.txt','a');
fprintf(file_data, 'LED Current: %d Amps',led_current);
fclose(file_data);
dlmwrite('SensorData.txt',space1,'delimiter',' ','newline', 'pc','-append')
...
%%
avgAm=mean(data_array(:,5)); 


%Confimation of data acquisition:
fprintf("Average Ambient Rate: %d\n\n",avgAm)
